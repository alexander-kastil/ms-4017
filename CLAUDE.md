...
- `demos/` — all course content, numbered by delivery order. Do not reorganize or renumber.
  ...
  - `03-extend` — `01-extensibility-fundamentals` (`01-options-taxonomy`, `02-copilot-connectors`, `03-decision-criteria`), `02-development-path` (`01-agent-builder`, `02-declarative-toolkit`, `03-copilot-studio-lowcode`, `04-custom-engine-sdk`, `05-foundry-agent-framework`, `06-publish-to-teams`), `03-manage-extensibility` (`01-agents-admin-center`, `02-graph-connector`, `03-agent-365-registry`, `04-entra-agent-id`, `05-purview-for-agents`)
...
- `02-development-path/02-declarative-toolkit/HelloWorldAgent` — declarative agent app package.
- `02-development-path/01-agent-builder` — Copilot Studio agent builder walkthrough (returns agent) plus its grounding document.
- `02-development-path/04-custom-engine-sdk/weather-agent` — TypeScript custom engine agent on the Microsoft 365 Agents SDK (`@microsoft/agents-hosting`) with LangChain/LangGraph and Express. Node 18/20/22, `npm run dev` / `npm run build`.
- `03-manage-extensibility/02-graph-connector/Github Issues` — TypeScript Azure Functions Microsoft 365 Copilot connector.
